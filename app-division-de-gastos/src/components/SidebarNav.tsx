...
const navItems = [
  ...
  { icon: 'groups', label: 'Grupos', href: '/dashboard/groups' },
  { icon: 'expenses', label: 'Gastos', href: '/dashboard/expenses' },
  { icon: 'settle', label: 'Saldar', href: '/dashboard/settle' },
  { icon: 'activity', label: 'Actividad', href: '/dashboard/activity' },
]

export function NavIcon({ type, size = 18 }: { type: string; size?: number }) {
  const s = `${size}`
  switch (type) {
    case 'dashboard':
      return (
